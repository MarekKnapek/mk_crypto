...
void mk_win_md2_init(struct mk_win_md2_state_s* self)
{
	assert(self);

	BCRYPT_ALG_HANDLE alg_prov;
	NTSTATUS got_alg_prov = BCryptOpenAlgorithmProvider(&alg_prov, BCRYPT_MD2_ALGORITHM, NULL, 0);
	if(got_alg_prov != 0)
	{
		exit(EXIT_FAILURE);
	}
...
void mk_win_md2_append(struct mk_win_md2_state_s* self, void const* data, size_t len)
{
	assert(self);
...
void mk_win_md2_finish(struct mk_win_md2_state_s* self, void* digest)
{
	assert(self);
	assert(digest);
