...
void mk_win_md5_init(struct mk_win_md5_state_s* self)
{
	...
	NTSTATUS got_alg_prov = BCryptOpenAlgorithmProvider(&alg_prov, BCRYPT_MD5_ALGORITHM, NULL, 0);
	if(got_alg_prov != 0)
	{
		exit(EXIT_FAILURE);
	}
...
void mk_win_md5_append(struct mk_win_md5_state_s* self, void const* data, size_t len)
{
	assert(self);
