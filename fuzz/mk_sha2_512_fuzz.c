...
#include "../src/hash/hash/mk_hash_hash_win_sha2_512.h"
...
{
	struct mk_hash_hash_win_sha2_512_s win_sha2_512;
	unsigned char win_sha2_512_digest[64];

	struct mk_hash_hash_sha2_512_s sha2_512;
	unsigned char sha2_512_digest[64];

	mk_hash_hash_win_sha2_512_init(&win_sha2_512);
	mk_hash_hash_win_sha2_512_append(&win_sha2_512, data, size);
	mk_hash_hash_win_sha2_512_finish(&win_sha2_512, &win_sha2_512_digest);

	mk_hash_hash_sha2_512_init(&sha2_512);
	mk_hash_hash_sha2_512_append(&sha2_512, data, size);
	mk_hash_hash_sha2_512_finish(&sha2_512, &sha2_512_digest);
...
static mk_inline void mk_sha2_512_fuzz_complex(unsigned char const* data, int size)
{
	...
	for(i = 0; i != parts; ++i)
	{
		...
		mk_hash_hash_win_sha2_512_append(&win_sha2_512, data, part_len);
		mk_hash_hash_sha2_512_append(&sha2_512, data, part_len);
		data += part_len;
		size -= part_len;
	}
	mk_hash_hash_win_sha2_512_finish(&win_sha2_512, &win_sha2_512_digest);
