...
static mk_inline int mk_fuzz_kdf_pbkdf2_data_gud(unsigned char const* data, int size, int* len1_out, int* len2_out, unsigned char const** data1_out, unsigned char const** data2_out, long* len3_out, int* len4_out)
{
	int len1;
	int len2;
...
void mk_fuzz_kdf_pbkdf2(unsigned char const* data, int size)
{
	static enum hash_e const s_algs1[] =
	{
		mk_hash_e_md2,
		mk_hash_e_md4,
		mk_hash_e_md5,
	...
	mk_assert((sizeof(s_algs1) / sizeof(s_algs1[i])) == (sizeof(s_algs2) / sizeof(s_algs2[i])));
	if(!mk_fuzz_kdf_pbkdf2_data_gud(data, size, &len1, &len2, &data1, &data2, &iterations, &key_len))
	{
		return;
	}
	n = (int)(sizeof(s_algs1) / sizeof(s_algs1[i]));
	for(i = 0; i != n; ++i)
	{
		...
		test(memcmp(key1, key2, key_len) == 0);
	}
}
