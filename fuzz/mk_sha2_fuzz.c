...
static int g_baseline_checked = 0;


static inline void mk_sha2_256_224_fuzz_baseline(void)
{
	...
	for(int i = 0; i != sizeof(s_tasks) / sizeof(s_tasks[0]); ++i)
	{
		...
		struct mk_sha2_256_224_state_s mk_sha2_256_224_state;
		...
		test(memcmp(mk_sha2_256_224_digest, output, 28) == 0);
	}
}
...
static inline void mk_sha2_256_fuzz(unsigned char const* data, size_t size)
{
	...
	test(memcmp(mk_sha2_256_digest, mk_win_sha2_256_digest, 32) == 0);
}

static inline void mk_sha2_512_224_fuzz_baseline(void)
{
	static char const s_input_0[] = "abc";
	...
	for(int i = 0; i != sizeof(s_tasks) / sizeof(s_tasks[0]); ++i)
	{
		...
		struct mk_sha2_512_224_state_s mk_sha2_512_224_state;
		...
		test(memcmp(mk_sha2_512_224_digest, output, 28) == 0);
	}
}

static inline void mk_sha2_512_256_fuzz_baseline(void)
{
	static char const s_input_0[] = "abc";
	static char const s_input_1[] = "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu";
	...
	for(int i = 0; i != sizeof(s_tasks) / sizeof(s_tasks[0]); ++i)
	{
		...
		struct mk_sha2_512_256_state_s mk_sha2_512_256_state;
		...
		test(memcmp(mk_sha2_512_256_digest, output, 32) == 0);
	}
}

static inline void mk_sha2_512_348_fuzz(unsigned char const* data, size_t size)
{
	struct mk_sha2_512_384_state_s mk_sha2_512_384_state;
	unsigned char mk_sha2_512_384_digest[48];

	struct mk_win_sha2_512_384_state_s mk_win_sha2_512_384_state;
	...
	test(memcmp(mk_sha2_512_384_digest, mk_win_sha2_512_384_digest, 48) == 0);
}
...
static inline void mk_sha2_512_fuzz(unsigned char const* data, size_t size)
{
	...
	test(memcmp(mk_sha2_512_digest, mk_win_sha2_512_digest, 64) == 0);
}

static inline void check_baseline(void)
{
	mk_sha2_256_224_fuzz_baseline();
	mk_sha2_512_224_fuzz_baseline();
	mk_sha2_512_256_fuzz_baseline();
}

static inline void check_regular(unsigned char const* data, size_t size)
{
...
void mk_sha2_fuzz(unsigned char const* data, size_t size)
{
	if(!g_baseline_checked)
	{
		check_baseline();
		g_baseline_checked = 1;
	}
	check_regular(data, size);
}
