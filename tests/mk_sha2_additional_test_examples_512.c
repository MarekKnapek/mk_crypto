...
int mk_sha2_additional_test_examples_512(void)
{
	...
	for(i = 0; i != sizeof(s_tasks) / sizeof(s_tasks[0]); ++i)
	{
		...
		if(pattern_len / 2 == 1 && repeat >= 1024)
		{
			...
			for(j = 0; j != blocks; ++j)
			{
				mk_hash_hash_sha2_512_append(&sha2_512, block, block_len);
			}
			mk_hash_hash_sha2_512_append(&sha2_512, block, repeat - blocks * block_len);
			free(block);
		}
		else
		{
			for(j = 0; j != repeat; ++j)
			{
				mk_hash_hash_sha2_512_append(&sha2_512, pattern_bin, pattern_len / 2);
			}
		}
		mk_hash_hash_sha2_512_finish(&sha2_512, &computed);
