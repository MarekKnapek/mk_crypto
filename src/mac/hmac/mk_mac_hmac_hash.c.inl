...
#define alg_id mk_mac_hmac_hash_alg_id
...
mk_jumbo void hmac_init(hmac_s* hmac, void const* key, int key_len)
{
	...
	mk_assert(key_len >= 0);
	mk_assert(digest_len <= block_len);
	...
	if(key_len > block_len)
	{
		...
		memset(ipad + digest_len, 0x00, (size_t)(block_len - digest_len));
	}
	else
	{
...
mk_jumbo void hmac_finish(hmac_s* hmac, void* digest)
{
	...
	hash_append(&hmac->m_hash, digest, digest_len);
	hash_finish(&hmac->m_hash, digest);
}
