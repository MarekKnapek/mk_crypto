...
static mk_inline void mk_sha3_base_detail_rho(struct mk_uint64_s state[25])
{
	int x;
	int y;
	int t;
...
static mk_inline void mk_sha3_base_detail_pi(struct mk_uint64_s in[25], struct mk_uint64_s out[25])
{
	int x;
	int y;
	int target_x;
...
static mk_inline void mk_sha3_base_detail_chi(struct mk_uint64_s in[25], struct mk_uint64_s out[25])
{
	int y;
	int x;
...
static mk_inline unsigned mk_sha3_base_detail_rc_next_bit(unsigned* rc)
{
	unsigned r;
	unsigned ret;
...
static mk_inline void mk_sha3_base_detail_rc_next_num(struct mk_uint64_s* out, unsigned* rc)
{
	struct mk_uint64_s tmp;
	...
	mk_uint64_zero(out);
	mk_uint64_from_int(&tmp, mk_sha3_base_detail_rc_next_bit(rc)); mk_uint64_shl(&tmp, &tmp,  0); mk_uint64_or(out, out, &tmp);
	mk_uint64_from_int(&tmp, mk_sha3_base_detail_rc_next_bit(rc)); mk_uint64_shl(&tmp, &tmp,  1); mk_uint64_or(out, out, &tmp);
	mk_uint64_from_int(&tmp, mk_sha3_base_detail_rc_next_bit(rc)); mk_uint64_shl(&tmp, &tmp,  3); mk_uint64_or(out, out, &tmp);
	mk_uint64_from_int(&tmp, mk_sha3_base_detail_rc_next_bit(rc)); mk_uint64_shl(&tmp, &tmp,  7); mk_uint64_or(out, out, &tmp);
	mk_uint64_from_int(&tmp, mk_sha3_base_detail_rc_next_bit(rc)); mk_uint64_shl(&tmp, &tmp, 15); mk_uint64_or(out, out, &tmp);
	mk_uint64_from_int(&tmp, mk_sha3_base_detail_rc_next_bit(rc)); mk_uint64_shl(&tmp, &tmp, 31); mk_uint64_or(out, out, &tmp);
	mk_uint64_from_int(&tmp, mk_sha3_base_detail_rc_next_bit(rc)); mk_uint64_shl(&tmp, &tmp, 63); mk_uint64_or(out, out, &tmp);
}

static mk_inline void mk_sha3_base_detail_iota(struct mk_uint64_s state[25], unsigned* rc)
{
	...
	mk_sha3_base_detail_rc_next_num(&rc_num, rc);
	mk_uint64_xor(&state[0], &state[0], &rc_num);
}

static mk_inline void mk_sha3_base_detail_rnd(struct mk_uint64_s state[25], unsigned* rc)
{
	...
	mk_sha3_base_detail_theta(state);
	mk_sha3_base_detail_rho(state);
	mk_sha3_base_detail_pi(state, tmp);
	mk_sha3_base_detail_chi(tmp, state);
	mk_sha3_base_detail_iota(state, rc);
}

static mk_inline void mk_sha3_base_detail_keccak_p(struct mk_uint64_s state[25])
{
	unsigned rc;
	int ir;
	...
	for(ir = 0; ir != 24; ++ir)
	{
		mk_sha3_base_detail_rnd(state, &rc);
	}
}

static mk_inline void mk_sha3_base_detail_keccak_f(struct mk_uint64_s state[25])
{
	mk_assert(state);

	mk_sha3_base_detail_keccak_p(state);
}

static mk_inline void mk_sha3_base_detail_mix_block(struct mk_uint64_s state[25], int block_len, void const* block)
{
...
mk_jumbo void mk_sha3_base_detail_init(struct mk_sha3_base_detail_s* sha3_base_detail)
{
	...
	mk_assert(sha3_base_detail);

	for(i = 0; i != 25; ++i)
	{
		mk_uint64_zero(&sha3_base_detail->m_state[i]);
	}
}
...
mk_jumbo void mk_sha3_base_detail_append_blocks(struct mk_sha3_base_detail_s* sha3_base_detail, int block_len, int nblocks, void const* pblocks)
{
	...
	{
		mk_sha3_base_detail_mix_block(sha3_base_detail->m_state, block_len, input);
		mk_sha3_base_detail_keccak_f(sha3_base_detail->m_state);
	}
}
...
mk_jumbo void mk_sha3_base_detail_finish(struct mk_sha3_base_detail_s* sha3_base_detail, int block_len, void* block, int idx, enum mk_sha3_base_detail_domain_e domain, int digest_len, void* digest)
{
	...
	mk_sha3_base_detail_append_blocks(sha3_base_detail, block_len, 1, input);
	to_copy = block_len < remaining ? block_len : remaining;
	for(i = 0; i != 25; ++i) mk_uint64_to_buff_le(&sha3_base_detail->m_state[i], tmp + i * 8);
	...
	{
		mk_sha3_base_detail_keccak_f(sha3_base_detail->m_state);
		for(j = 0; j != 25; ++j) mk_uint64_to_buff_le(&sha3_base_detail->m_state[j], tmp + j * 8);
		memcpy(output, tmp, to_copy);
	}
