...
void mk_sha1_base_init(struct mk_sha1_base_s* sha1_base)
{
	...
	sha1_base->m_state[0] = mk_sha1_base_detail_init[0];
	sha1_base->m_state[1] = mk_sha1_base_detail_init[1];
	sha1_base->m_state[2] = mk_sha1_base_detail_init[2];
	sha1_base->m_state[3] = mk_sha1_base_detail_init[3];
	sha1_base->m_state[4] = mk_sha1_base_detail_init[4];
	mk_uint64_zero(&sha1_base->m_len);
}
...
void mk_sha1_base_append_blocks(struct mk_sha1_base_s* sha1_base, int nblocks, void const* pblocks)
{
	...
	mk_uint64_add(&sha1_base->m_len, &sha1_base->m_len, &len_bytes);
	...
	{
		h[0] = sha1_base->m_state[0];
		h[1] = sha1_base->m_state[1];
		h[2] = sha1_base->m_state[2];
		h[3] = sha1_base->m_state[3];
		h[4] = sha1_base->m_state[4];
		for(i = 0; i != 16; ++i)
		{
		...
		mk_uint32_add(&sha1_base->m_state[0], &sha1_base->m_state[0], &h[0]);
		mk_uint32_add(&sha1_base->m_state[1], &sha1_base->m_state[1], &h[1]);
		mk_uint32_add(&sha1_base->m_state[2], &sha1_base->m_state[2], &h[2]);
		mk_uint32_add(&sha1_base->m_state[3], &sha1_base->m_state[3], &h[3]);
		mk_uint32_add(&sha1_base->m_state[4], &sha1_base->m_state[4], &h[4]);
	}
}
...
void mk_sha1_base_finish(struct mk_sha1_base_s* sha1_base, void* block, int idx, void* digest)
{
	...
	mk_uint64_add(&len, &len, &sha1_base->m_len);
	...
	if(capacity >= 8)
	{
		...
		mk_sha1_base_append_blocks(sha1_base, 1, input);
	}
	else
	{
		...
		mk_sha1_base_append_blocks(sha1_base, 1, buff);
	}
	mk_uint32_to_buff_be(&sha1_base->m_state[0], output + 0 * 4);
	mk_uint32_to_buff_be(&sha1_base->m_state[1], output + 1 * 4);
	mk_uint32_to_buff_be(&sha1_base->m_state[2], output + 2 * 4);
	mk_uint32_to_buff_be(&sha1_base->m_state[3], output + 3 * 4);
	mk_uint32_to_buff_be(&sha1_base->m_state[4], output + 4 * 4);
}
