...
mk_jumbo void mk_hash_base_hash_sha3_384_append_blocks(struct mk_hash_base_hash_sha3_384_s* self, void const* pblocks, int nblocks)
{
	mk_assert(self);
	mk_assert(pblocks || nblocks == 0);
	mk_assert(nblocks >= 0);

	mk_hash_base_detail_sha3_append_blocks(&self->m_sha3, mk_hash_base_hash_sha3_384_block_len, pblocks, nblocks);
}
