#include "mk_hash_base_hash_md2.h"
...
static unsigned char const s_mk_md2_base_detail_table[256] =
{
...
mk_jumbo void mk_hash_base_hash_md2_append_blocks(struct mk_hash_base_hash_md2_s* self, void const* pblocks, int nblocks)
{
	...
	for(iblock = 0; iblock != nblocks; ++iblock, input += 16)
	{
		...
		for(j = 0; j != 18; ++j)
		{
			for(k = 0; k != 48; ++k)
			{
				t = x[k] ^= s_mk_md2_base_detail_table[t];
			}
			t = (t + j) & 0xff;
		}
		...
		for(j = 0; j != 16; ++j)
		{
			l = self->m_checksum[j] ^= s_mk_md2_base_detail_table[input[j] ^ l];
		}
	}
	memcpy(self->m_state, x + 0 * 16, 16);
}
