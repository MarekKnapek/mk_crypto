...
mk_jumbo void mk_win_sha2_384_init(struct mk_win_sha2_384_s* win_sha2_384)
{
	...
	win_sha2_384->m_csp = csp;
	win_sha2_384->m_hash = hash;
}
...
mk_jumbo void mk_win_sha2_384_append(struct mk_win_sha2_384_s* win_sha2_384, void const* msg, int msg_len)
{
	...
	hashed = CryptHashData(win_sha2_384->m_hash, msg, msg_len, 0);
...
mk_jumbo void mk_win_sha2_384_finish(struct mk_win_sha2_384_s* win_sha2_384, void* digest)
{
	...
	digest_len = 48;
	got_hash = CryptGetHashParam(win_sha2_384->m_hash, HP_HASHVAL, digest, &digest_len, 0);
	mk_assert(got_hash != FALSE);
	mk_assert(digest_len == 48);

	hash_freed = CryptDestroyHash(win_sha2_384->m_hash);
	mk_assert(hash_freed != FALSE);

	csp_freed = CryptReleaseContext(win_sha2_384->m_csp, 0);
