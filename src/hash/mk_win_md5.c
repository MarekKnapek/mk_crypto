...
mk_jumbo void mk_win_md5_init(struct mk_win_md5_s* win_md5)
{
	...
	hash_created = CryptCreateHash(csp, CALG_MD5, 0, 0, &hash);
...
mk_jumbo void mk_win_md5_append(struct mk_win_md5_s* win_md5, void const* msg, int msg_len)
{
	...
	hashed = CryptHashData(win_md5->m_hash, msg, msg_len, 0);
...
mk_jumbo void mk_win_md5_finish(struct mk_win_md5_s* win_md5, void* digest)
{
	...
	digest_len = 16;
	got_hash = CryptGetHashParam(win_md5->m_hash, HP_HASHVAL, digest, &digest_len, 0);
	mk_assert(got_hash != FALSE);
	mk_assert(digest_len == 16);

	hash_freed = CryptDestroyHash(win_md5->m_hash);
	mk_assert(hash_freed != FALSE);

	csp_freed = CryptReleaseContext(win_md5->m_csp, 0);
