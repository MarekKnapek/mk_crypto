...
mk_jumbo void mk_sha3_shake256_block_append(struct mk_sha3_shake256_base_s* sha3_shake256_base, void* block, int* pidx, void const* msg, int msg_len)
{
	...
	mk_assert(block);
	mk_assert(pidx && *pidx >= 0 && *pidx < mk_sha3_shake256_base_block_len);
	...
	remaining = msg_len;
	capacity = mk_sha3_shake256_base_block_len - idx;
	if(remaining >= capacity)
	{
		...
		blocks = remaining / mk_sha3_shake256_base_block_len;
		mk_sha3_shake256_base_append_blocks(sha3_shake256_base, blocks, input);
		input += blocks * mk_sha3_shake256_base_block_len;
		remaining -= blocks * mk_sha3_shake256_base_block_len;
	}
