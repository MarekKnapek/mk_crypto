...
mk_jumbo void mk_md2_block_append(struct mk_md2_base_s* md2_base, void* block, int* pidx, void const* msg, int msg_len)
{
	...
	mk_assert(md2_base);
	mk_assert(block);
	mk_assert(pidx && *pidx >= 0 && *pidx < mk_md2_base_block_len);
	...
	remaining = msg_len;
	capacity = mk_md2_base_block_len - idx;
	if(remaining >= capacity)
	{
		...
		blocks = remaining / mk_md2_base_block_len;
		mk_md2_base_append_blocks(md2_base, blocks, input);
		input += blocks * mk_md2_base_block_len;
		remaining -= blocks * mk_md2_base_block_len;
	}
