...
mk_jumbo void mk_sha2_512_block_append(struct mk_sha2_512_base_s* sha2_512_base, void* block, int* pidx, void const* msg, int msg_len)
{
	...
	mk_assert(block);
	mk_assert(pidx && *pidx >= 0 && *pidx < mk_sha2_512_base_block_len);
	...
	remaining = msg_len;
	capacity = mk_sha2_512_base_block_len - idx;
	if(remaining >= capacity)
	{
		...
		blocks = remaining / mk_sha2_512_base_block_len;
		mk_sha2_512_base_append_blocks(sha2_512_base, blocks, input);
		input += blocks * mk_sha2_512_base_block_len;
		remaining -= blocks * mk_sha2_512_base_block_len;
	}
