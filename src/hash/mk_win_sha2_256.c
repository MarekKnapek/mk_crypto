...
mk_jumbo void mk_win_sha2_256_init(struct mk_win_sha2_256_s* win_sha2_256)
{
	...
	hash_created = CryptCreateHash(csp, CALG_SHA_256, 0, 0, &hash);
	mk_assert(hash_created != FALSE);

	win_sha2_256->m_csp = csp;
	win_sha2_256->m_hash = hash;
}
...
mk_jumbo void mk_win_sha2_256_append(struct mk_win_sha2_256_s* win_sha2_256, void const* msg, int msg_len)
{
	...
	hashed = CryptHashData(win_sha2_256->m_hash, msg, msg_len, 0);
...
mk_jumbo void mk_win_sha2_256_finish(struct mk_win_sha2_256_s* win_sha2_256, void* digest)
{
	...
	digest_len = 32;
	got_hash = CryptGetHashParam(win_sha2_256->m_hash, HP_HASHVAL, digest, &digest_len, 0);
	mk_assert(got_hash != FALSE);
	mk_assert(digest_len == 32);

	hash_freed = CryptDestroyHash(win_sha2_256->m_hash);
	mk_assert(hash_freed != FALSE);

	csp_freed = CryptReleaseContext(win_sha2_256->m_csp, 0);
