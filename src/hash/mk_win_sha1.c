...
mk_jumbo void mk_win_sha1_append(struct mk_win_sha1_s* win_sha1, void const* msg, int msg_len)
{
	...
	hashed = CryptHashData(win_sha1->m_hash, msg, msg_len, 0);
...
mk_jumbo void mk_win_sha1_finish(struct mk_win_sha1_s* win_sha1, void* digest)
{
	...
	digest_len = 20;
	got_hash = CryptGetHashParam(win_sha1->m_hash, HP_HASHVAL, digest, &digest_len, 0);
	mk_assert(got_hash != FALSE);
	mk_assert(digest_len == 20);

	hash_freed = CryptDestroyHash(win_sha1->m_hash);
	mk_assert(hash_freed != FALSE);

	csp_freed = CryptReleaseContext(win_sha1->m_csp, 0);
